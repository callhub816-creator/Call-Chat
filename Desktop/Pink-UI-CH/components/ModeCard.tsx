...
import { Sparkles, Heart, Flame, Cloud, Zap } from 'lucide-react';
...
const ModeCard: React.FC<ModeCardProps> = ({ data, onClick }) => {
  ...
  return (
    <div 
      onClick={onClick}
      className="group relative w-full aspect-[3/4] max-w-[320px] mx-auto perspective-1000 cursor-pointer"
      role="button"
    ...
    >
      ...
      <div 
        className={`
          ...
          group-hover:shadow-[0_30px_60px_-12px_rgba(178,141,255,0.5)]
        `}
      >
        ...
        <div className="relative z-10 w-full h-full flex flex-col justify-end p-8">
          ...
          {/* Text Container with slight glass backdrop for readability */}
          <div className="
            flex flex-col gap-2 
            p-6 rounded-2xl 
          ...
          {/* Call to Action Indicator (Visible on hover) */}
          <div className="absolute bottom-8 right-8 opacity-0 group-hover:opacity-100 transition-opacity duration-500 translate-y-2 group-hover:translate-y-0">
             <div className="px-4 py-1.5 rounded-full bg-white text-[#5e3a58] text-[10px] font-bold uppercase tracking-widest shadow-md">
                Create
             </div>
          </div>
